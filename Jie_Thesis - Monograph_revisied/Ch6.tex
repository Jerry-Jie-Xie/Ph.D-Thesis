...
This chapter presents a method for the classification of simultaneously vocalising frog species in low SNR recordings. In Chapters \ref{cha:cha4EnhancedFeature} and \ref{cha:cha5WaveletFeature}, frog call classification is solved using a SISL framework, which cannot reflect the nature of automatically collected environmental recordings. Most automatically collected field recordings have low SNR and contain multiple simultaneous animal vocal activities including frogs, birds, insects, and so on. This attribute of environmental recordings makes MIML learning a suitable classification framework. Specifically, frog syllables in one audio clip (such as 10-second) are regarded as \textit{multiple instance}, and the frog species included in that audio clip denotes \textit{multiple labels}. 
The key part of this MIML classification framework for studying frog recordings is to detect individual syllables in environmental recordings with multiple simultaneously vocalising frog species. 
After syllable detection, acoustic features are extracted from each individual syllable. Then, three MIML classifiers are used for classifying each 10-second recording.
...
\textit{Litoria fallax}              & LFX           & 4000                                                        &
\textit{Litoria latopalmata}              & LLA           & 4000                                                        &
         280                   \\ 
...
\noindent To reduce the graininess and remove small gaps within
each individual acoustic event, a spectrogram image is convolved
with a Gaussian kernel. The size of the Gaussian kernel is experimentally set at 15 after the consideration of the trade-off between removing the background graininess and blurring different acoustic events.
\\
\textbf{Step 2:} Spectral subtraction

\noindent Gaussian filtering can successfully remove the graininess, but some noises, such as wind, insect, motor engine that cover the whole recording, can not be addressed using Gaussian filtering. Here, spectral subtraction is used to deal with those noises. 


\begin{algorithm}
\DontPrintSemicolon
\KwData{$\hat{S_{tf}}$, spectrogram after Gaussian filtering.}
