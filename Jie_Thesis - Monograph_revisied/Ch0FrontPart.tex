...
\begin{enumerate} 
\item[(1)]	Develop an enhanced feature set for frog call classification (Chapter \ref{cha:cha4EnhancedFeature}). 
\\
Time-frequency information of frog calls can be effectively represented via an enhanced representation of temporal, perceptual and cepstral features. The classification performance of various machine learning techniques is compared with different feature sets. Our proposed enhanced feature set achieves a satisfied classification accuracy.
 
\item[(2)]	Propose a novel feature based on adaptive wavelet packet decomposition (Chapter \ref{cha:cha5WaveletFeature}). 
\\
...
\\
For the ML learning, acoustic features are first calculated without segmentation. Then, ML learning is used to classify multiple simultaneously vocalising frog species using extracted features. Three ML learning methods are compared: Binary relevance, Classifier Chains, Random k-labelsets, where the base classifier is a decision tree classifier. Furthermore, the frog calling activity and species richness over three months are estimated based on the results of acoustic event detection and ML classification, respectively. 
Lastly, the correlation analysis between frog calling activity/species richness and weather variables (mean temperature and rainfall) are studied to demonstrate the application of our proposed ML classification framework.
\end{enumerate}
