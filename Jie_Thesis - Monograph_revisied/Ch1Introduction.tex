...
Experimental results demonstrate that MIML learning can classifying multiple frog species in low SNR recordings with a hamming loss of 0.1192, which is 2.70 times better than the non-informative classifier. Experimental results also show that the proposed MIML classification framework can achieve a better performance compared to the SISL classification.
\\
...
\item \textbf{Xie, Jie}, Towsey, Michael, Zhang, Jinglan, and Roe, Paul, Frog call classification: a survey, Artificial Intelligence Review (\textbf{Under review with small revision})
